...
def get_tics(pth):
    ...
    tics = [''.join(tic.split()) for tic in tics]
    tics = [tic.lower() for tic in tics if tic != '']
...
def read_dat(tic):
    ...
    """
    file_pth = os.path.join(DATDIR, f'{tic}_prc.dat')
    if not os.path.exists(file_pth):
        raise ValueError(f'File path {file_pth} does not exist!')

    with open(file_pth) as fobj:
        file_lines = fobj.readlines()

    list_lines = [line.rstrip() for line in file_lines]
...
def line_to_dict(line):
    ...
    for colname in COLUMNS:
        end_width = cum_width + COLWIDTHS[colname]
        data_per_line[colname] = line[cum_width:end_width]
        cum_width = end_width
