...
def get_tics(pth):
    ...
    """
    # <COMPLETE THIS PART>
    with open(pth) as ticker_text:
        ticker_list = [ticker.strip().lower() for ticker in ticker_text]
    return ticker_list
# ---------------------------------------------------------------------------- 
#   Please complete the body of this function so it matches its docstring
#   description. See the assessment description file for more information.
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    stock_data_location = os.path.join(DATDIR, f"{tic}_prc.dat")
    with open(stock_data_location) as stock_data:
        stock_data_list = [info.strip() for info in stock_data]
...
def line_to_dict(line):
    ...
    for i in range(len(COLUMNS)):
        dict[COLUMNS[i]] = line[accum:accum + COLWIDTHS[COLUMNS[i]]]
        accum = accum + int(COLWIDTHS[COLUMNS[i]])
    return dict
